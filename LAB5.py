...
def passGen():
    while True:
        user_number = int(input("Enter a number between 3 and 7 (inclusive):\n"))
        try:
            if 3 <= user_number <= 7:
                string = ""
                password = ""
                for j in range(user_number):
                    tempPassword = random.choice(words)
                    string = string + tempPassword
                password = string[::-1]
                return password
            else:
                print("Invalid input! Please enter a number between 3 and 7 (inclusive):\n")
        except ValueError:
            print("Invalid input! Please enter a number between 3 and 7 (inclusive):\n")
...
def rep_with_upper(string):
    ...
    new_string = string.replace(char, char.upper())
    return new_string
...
# Calling passGen function with user input and assigning it to a string.
password = passGen()

# Calling the rep_with_upper function with the password returned from passGen as an argument value and assigning it to a new  string.
new_password = rep_with_upper(password)

# Calling the swap_letter function with the new password returned from rep_with_upper function.
final_password = swap_letter(new_password)

print("Initial password: ", password, "\n")
print("Final version of the password: ", final_password, "\n")


def search_letter(s1, s2):
    return s2 in s1


# Calling the search_letter function with the final password and my name's (Ahmet) first letter 'A' and assigning it to a variable to see the result.
result = search_letter(final_password, 'A')

# It is returning false when final_password contains the lowercase letter but I hope you want it that way.
print(result)
